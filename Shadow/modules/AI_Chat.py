...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On" or status == "oN":
        lel = await edit_or_reply(message, "Processing...")
        lol = add_chat(int(message.chat.id))
        if not lol:
            await lel.edit("United AI Already Activated In This Chat")
            return
        await lel.edit(
            f"united AI Successfully Added For Users In The Chat {message.chat.id}"
        )
        ...
        if not Escobar:
            await lel.edit("united AI Was Not Activated In This Chat")
            return
        await lel.edit(
            f"united AI Successfully Deactivated For Users In The Chat {message.chat.id}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        test = msg
        test = test.replace("shadow", "Shadow")
        test = test.replace("Shadow", "shadow")
        response = await lunaQuery(
            test, message.from_user.id if message.from_user else 0
        )
        response = response.replace("Shadow", "shadow")
        response = response.replace("shadow", "Shadow")
...
async def deshadeeth(client, message):
    msg = message.text
    if msg.startswith("/") or msg.startswith("@"):
        message.continue_propagation()
    u = msg.split()
...
__help__ = """
<b><i>united AI 3.0 IS THE ONLY AI SYSTEM WHICH CAN DETECT & REPLY UPTO 200 LANGUAGES</i></b>
