...
from Shadow.events import register
from Shadow import OWNER_ID
from Shadow import telethn as tbot
...
async def lego(event):
 ...
 try:
    ...
    fname="LogoMakeBy_MRfriendly.png"
    img.save(fname, "png")
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./Shadow/pack/blackimg.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./Shadow/pack/Knife.otf", 1000)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./Shadow/pack/fjoker.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "white"
    shadowcolor = "blue"
    font = ImageFont.truetype("./Shadow/pack/Gothic-Joker.ttf", 500)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./Shadow/pack/2.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "PaleGreen"
    shadowcolor = "blue"
    font = ImageFont.truetype("./Shadow/pack/Maghrib.ttf", 800)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
