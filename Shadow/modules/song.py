...
from Shadow.function.pluginhelpers import get_text, progress
...
async def ytmusic(client, message: Message):
    ...
    if dl_limit >= 5:
        await message.reply_text(
            "my server busy due to too many downloads, try again after sometime."
        )
        return
...
async def jssong(_, message):
    ...
    if dl_limit >= 3:
        await message.reply_text(
            "mys server busy due to too many downloads, try again after sometime."
        )
        return
    if is_downloading:
        await message.reply_text(
